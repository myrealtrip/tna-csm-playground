...
> 작성: 2026-03-20 · 최초 개발: 시은 + 기원 + Claude Code
> 이 문서는 프로젝트를 이어받을 동료를 위한 전체 컨텍스트입니다.
...
| 개선 | 설명 |
|------|------|
| 파트너 등급 자동 산정 | 확정률 + 응답시간 + 취소 패턴 → A/B/C 등급 |
| 이상 탐지 알림 | 갑작스러운 취소율 급증, 응답 지연 패턴 감지 |
| 다국어 리포트 | 파트너에게 직접 공유할 수 있도록 영어/일어 버전 |
...
이 프로젝트는 AI Lab의 동훈님, 수진님이 제안해주신 새로운 업무 방식에서 시작되었고,
단우님이 `tna-csm-playground` 레포를 마련해주셔서 빠르게 실험할 수 있었습니다.

콘솔(F12)을 열면 숨겨진 감사 메시지를 확인할 수 있습니다.
