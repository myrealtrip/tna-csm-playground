...
businessMinutes(startMs, endMs, utcOffset)  # 비업무시간 제외 응답시간 (v3~)
...
- 취소율 (3단계 분리, v3~)
  - **전체 취소율** = `cancel / total * 100`
  - **고객 사유 취소율** = 고객 사유 취소 / total
  - **순 취소율** (`netCancelRate`) = 파트너 귀책 취소 / total — 파트너 품질 평가에 사용
...
| 파트너 경유 취소 | 파트너, 운영사, 운영 불가 | v3에서 "파트너 요청으로 취소"를 리라벨 |
| 미분류 | 위 패턴에 해당 없음 | |
...
- `businessMinutes()` 함수로 비업무시간(21:00–09:00, 파트너 현지 시간 기준)을 제외한 응답시간 계산
- **파트너 시간대** (`utcOffset`) 파라미터 지원 — 기본값 KST(+9), 북마클릿에서 입력 (예: 파리 하절기=2, 뉴욕=-5)
- **중앙값** (`medianBusinessResponseMinutes`) 추가 — 평균은 이상치에 취약하므로 중앙값을 병기
- 새벽 문의 → 아침 답변 같은 정상 응답이 "8시간 지연"으로 왜곡되는 문제 해결
...
| Claude Code에서 대용량 MD 직접 분석 | 대화방 755개(약 985KB) → `prompt is too long` 반복 | 아래 "대용량 MD 컨텍스트 초과" 참조 |
| MD 추출기와 분석기 분리 운영 | 동일 API 2번 호출, 북마클릿 2개 관리 | 통합이 답 |

### 대용량 MD 컨텍스트 초과 (2026-03-20 해결)

**증상**: 대화방 755개(약 985KB) MD 파일을 Claude Code에서 분석 요청 시 `prompt is too long` 반복 발생.

**원인**: Claude Code 요금제에 따라 사용 가능한 컨텍스트 윈도우 크기가 다름. 대화방 수가 많을수록 파일이 커지므로, 요금제에 따라 한 번에 처리할 수 있는 대화방 수가 달라진다.

**해결**: 분석 프롬프트에 청크 읽기 지시 1줄 추가.

```
파일이 크면 "## N." 구분자 기준으로 나눠 읽고, 전체를 종합해서 분석해.
```

Claude Code는 `Read` 도구의 `offset`/`limit` 파라미터로 파일을 부분 읽기할 수 있으므로, 이 지시만으로 어떤 크기의 MD 파일이든 처리 가능. 파일 분할이나 별도 스크립트 불필요.

**교훈**: 복잡한 엔지니어링(파일 분할, 에이전트 파이프라인 등)보다 프롬프트 한 줄이 답인 경우가 있다.
...
| **v3 분석 고도화** | 1 | 취소율 3단계 분리(netCancelRate), 업무시간 응답시간(businessMinutes), 시간대 지원, 중앙값 |
| **대용량 MD 지원** | 3 | 프롬프트 청크 읽기 지시, 파일 경로 힌트, 대시보드 직링크 |
