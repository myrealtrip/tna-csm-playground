...
폴더 지정 시 → `--images` 옵션과 동일하게 이미지를 매핑한다 (신규 모드에서는 여기서 이미지 처리가 완료되므로 Step 4는 건너뜀).
...
- **골든 샘플 수정 금지**: `PDP_TF/Kiwon/pilots/*.html` 파일은 절대 수정하지 않는다
- **템플릿 직접 수정 금지**: `PDP_TF/Kiwon/templates/*.html` 원본을 건드리지 않는다. 항상 복사 후 치환
