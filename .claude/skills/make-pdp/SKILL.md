---
name: make-pdp
description: "MRT 상품 URL → PDP 상세페이지 HTML 자동 생성. 트리거 키워드: PDP 만들어, 상세페이지 생성, make-pdp, PDP 생성, 상세페이지 만들어줘, PDP HTML"
---

# make-pdp — MRT 상품 PDP 자동 생성

MRT 상품 URL을 입력하면 카테고리를 자동 판별하고, 해당 템플릿에 데이터를 채워 PDP HTML을 생성한다.

## 사용법

```
/make-pdp <URL>                        # 기본: 자동 판별 → HTML 생성
/make-pdp <URL> --exclusive            # PB Exclusive 카테고리 강제 지정
/make-pdp <URL> --category <카테고리>   # 카테고리 수동 지정 (tour/ticket/activity/convenience/semi-package)
...
공유 CSS 파일이 이미 존재: `output/pb-exclusive.css`
...
```bash
npx lighthouse "file://OUTPUT_DIR/{slug}.html" \
...
> `/autoresearch pdp-lighthouse target=output/{slug}.html`로 심층 최적화할 수 있어요.
...
```
mcp__chrome-devtools__new_page(url=file://OUTPUT_DIR/{slug}.html)
```
...
| `docs/make-pdp-spec.md` | 설계 스펙 문서 |
