...
공유 CSS 파일: `output/pb-exclusive.css`

```html
<link rel="stylesheet" href="pb-exclusive.css">
```

다른 카테고리 템플릿은 기존대로 인라인 CSS를 유지한다.
...
```
mcp__chrome-devtools__new_page(url=file://{absolute_path}/output/{slug}.html)
```
...
- **골든 샘플 수정 금지**: `pilots/*.html` 파일은 절대 수정하지 않는다
