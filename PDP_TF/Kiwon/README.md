...
cp -r tna-csm-playground/PDP_TF/Kiwon ~/my-pdp-maker   # 폴더명 자유
cd ~/my-pdp-maker
```

> 폴더명은 자유롭게 지정하세요. 모든 경로가 상대 경로라 어디서든 동작합니다.
...
로컬 폴더는 git 연결이 없으므로, 원본 레포에서 수동으로 업데이트합니다.

```bash
# 원본 레포 최신화
cd tna-csm-playground && git pull origin main

# 내 폴더에 덮어쓰기 (output/ 제외)
rsync -av --exclude='output/' PDP_TF/Kiwon/ ~/my-pdp-maker/
```

### 출력물 공유

생성한 PDP를 팀에 공유하려면 원본 레포에 복사 후 푸시:

```bash
cp ~/my-pdp-maker/output/my-pdp.html tna-csm-playground/PDP_TF/Kiwon/output/
